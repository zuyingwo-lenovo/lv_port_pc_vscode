    ...
    while (agent_running) {
        ...
        if (audio_sim_get_speech(&audio_data, &audio_size)) {
            ...
            if (lower_text.find("hey") != std::string::npos || lower_text.find("hey thinkpad") != std::string::npos || lower_text.find("think pad") != std::string::npos || lower_text.find("pink") != std::string::npos) {
                ...
                // std::string prompt = "You are a helpful voice assistant. Keep replies short. User says: " + transcribed_text;
                std::string prompt = transcribed_text;
                ...
                if (generator_exp) {
                    ...
                    if (generator.write(prompt) == HAILO_SUCCESS) {
                        ...
                        if (completion_exp) {
                            ...
                            std::cout << last_agent_text << std::endl;
                            ...
                            // Safely escape response using single quotes to prevent bash injection
                            ...
                            } else if (safe_device == "bluealsa" || safe_device.find("bluealsa:") == 0) {
                                // bluealsa device requires plug: adapter for automated resampling support
                                safe_device = "plug:" + safe_device;
                            }
