...
static std::string whisper_hef_path =
    "/usr/local/hailo/resources/models/hailo10h/Whisper-Base.hef";
static std::string llm_hef_path =
    "/usr/local/hailo/resources/models/hailo10h/Qwen2.5-1.5B-Instruct.hef";
  ...
  while (agent_running) {
    ...
    if (audio_sim_get_speech(&audio_data, &audio_size)) {
      ...
      if (transcribe_exp) {
        ...
        std::cout << "[Agent] Whisper Transcription: '" << transcribed_text
                  << "'" << std::endl;
      } else {
        std::cerr << "[Agent] Whisper error: " << transcribe_exp.status()
                  << std::endl;
      }
      ...
      // 2. Check wake word
      if (lower_text.find("hey") != std::string::npos ||
          lower_text.find("hey thinkpad") != std::string::npos ||
          lower_text.find("think pad") != std::string::npos ||
          lower_text.find("pink") != std::string::npos) {
        std::cout << "[Agent] Wake word detected!" << std::endl;
        // Wake word detected! Extract the rest of the sentence.
        // For simplicity, we just pass the transcribed text to the LLM
        // directly, the LLM can handle a prompt starting with "hey thinkpad"
        {
          std::lock_guard<std::mutex> lock(agent_mutex);
        ...
        std::string prompt = "You are a helpful voice assistant named "
                             "Thinkpad. Keep replies short. User says: " +
                             transcribed_text;
          ...
          if (generator.write(prompt) == HAILO_SUCCESS) {
            ...
            if (completion_exp) {
              ...
              while (completion.generation_status() ==
                ...
                auto token_exp =
                    completion.read(std::chrono::milliseconds(2000));
                if (token_exp) {
                  ...
                  if (++token_count > 100)
                    break; // Limit length for speed
                }
              }

              std::cout
                  << "\n[Agent] LLM Generation complete. Sending to TTS..."
                  << std::endl;
              // 4. TTS Playback
              {
                std::lock_guard<std::mutex> lock(agent_mutex);
              ...
              size_t pos = 0;
              while ((pos = safe_response.find("\"", pos)) !=
                     std::string::npos) {
                safe_response.replace(pos, 1, "\\\"");
                pos += 2;
              }
              // Use flite to generate wav and then play with aplay for better
              // ALSA support
              ...
              std::string tmp_wav = "voicewave_tts.wav";
              std::string cmd =
                  "flite -t \"" + safe_response + "\" -o " + tmp_wav + " && ";
              if (!safe_device.empty()) {
                cmd += "aplay -D " + safe_device + " -q " + tmp_wav;
              } else {
                cmd += "aplay -q " + tmp_wav;
              }
              cmd += " & del " + tmp_wav; // Windows uses del instead of rm
              std::cout << "[Agent] Executing TTS: " << cmd << std::endl;
              system(cmd.c_str());
              std::cout << "[Agent] TTS finished." << std::endl;
            } else {
              std::cerr << "[Agent] Failed to generate: "
                        << completion_exp.status() << std::endl;
            }
          } else {
            std::cerr << "[Agent] Failed to write to generator" << std::endl;
          }
        } else {
          std::cerr << "[Agent] Failed to create generator: "
                    << generator_exp.status() << std::endl;
        }
      } else {
...
static void agent_thread_wrapper() {
  ...
  std::cout << "[Agent Thread] Loading Speech2Text model from: "
            << whisper_hef_path << std::endl;
  ...
  if (s2t_exp) {
    ...
    std::cerr << "[Agent Thread] Failed to load S2T: " << s2t_exp.status()
              << std::endl;
  }

  std::cout << "[Agent Thread] Loading LLM model from: " << llm_hef_path
            << std::endl;
  ...
  if (llm_exp) {
    ...
    std::cerr << "[Agent Thread] Failed to load LLM: " << llm_exp.status()
              << std::endl;
  }

  std::cout << "[Agent Thread] Initialization complete! Entering main "
               "processing loop..."
            << std::endl;
...
extern "C" void agent_pipeline_init(const char *tts_device) {
  if (tts_device) {
    ...
    std::cout << "[Agent Init] Custom TTS audio device set to: "
              << tts_audio_device << std::endl;
  }
  std::cout << "[Agent Init] Starting background loading thread..."
            << std::endl;
  agent_running = true;
...
extern "C" void agent_pipeline_init(const char *tts_device) {
  std::cout << "[Agent Init] Simulated Agent Pipeline on Windows started (AI "
               "hardware bypassed)"
            << std::endl;
}
...
extern "C" agent_state_t agent_get_state(void) { return AGENT_STATE_WAITING; }

extern "C" const char *agent_get_last_user_text(void) {
  return "Windows Simulation";
}

extern "C" const char *agent_get_last_agent_text(void) {
  return "AI hardware bypassed.";
}
