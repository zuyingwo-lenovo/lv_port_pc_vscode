...
static void *audio_capture_thread(void *arg) {
  ...
  if ((pcm = snd_pcm_open(&pcm_handle, PCM_DEVICE, SND_PCM_STREAM_CAPTURE, 0)) <
      0) {
    ...
    if (pcm == -EPIPE) {
        ...
        if (is_recording_speech) {
          ...
          if (silence_frames >= SAMPLE_RATE) {
            ...
            if (speech_length > 0 && ready_speech == NULL) {
              ...
              memcpy(ready_speech, speech_buffer,
                     speech_length * sizeof(float));
      ...
      if (is_recording_speech) {
        if (speech_length + pcm > speech_capacity) {
          speech_capacity =
              speech_capacity == 0 ? 16000 * 5 : speech_capacity * 2;
          speech_buffer =
              (float *)realloc(speech_buffer, speech_capacity * sizeof(float));
        }
...
void audio_sim_init(void) {
  if (is_running == 0) {
    is_running = 1;
    if (pthread_create(&audio_thread_id, NULL, audio_capture_thread, NULL) !=
        0) {
...
uint8_t audio_sim_get_amplitude(void) { return current_amplitude; }
...
void audio_sim_init(void) {
  // Windows proxy placeholder: does nothing
}

uint8_t audio_sim_get_amplitude(void) { return 0; }

int audio_sim_is_listening(void) { return 0; }

int audio_sim_get_speech(float **out_buffer, size_t *out_size) { return 0; }
